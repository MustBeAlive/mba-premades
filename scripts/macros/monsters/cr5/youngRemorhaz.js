...
async function heatedBody({ speaker, actor, token, character, item, args, scope, workflow }) {
    ...
    let featureData = await mba.getItemFromCompendium("mba-premades.MBA Monster Features", "Young Remorhaz: Heated Body", false);
    if (!featureData) return;
    delete featureData._id;
    if (token.document.name === "Remorhaz") {
        featureData.name = "Remorhaz: Heated Body";
        featureData.system.damage.parts = [["3d6[fire]", "fire"]];
    }
