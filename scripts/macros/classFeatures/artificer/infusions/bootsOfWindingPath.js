...
export async function bootsOfWindingPath({ speaker, actor, token, character, item, args, scope, workflow }) {
    let boots = await mba.getItem(workflow.actor, "Boots of the Winding Path");
    if (!boots.system.equipped) {
        ui.notifications.warn("Boots of the Winding Path must be equipped!");
        await game.messages.get(workflow.itemCardId).delete();
        return;
    }
    if (boots.system.attunement != 2) {
        ui.notifications.warn("You must attune to this pair of boots in order to use its magic ability!");
        await game.messages.get(workflow.itemCardId).delete();
        return;
    }
    ...
    if (randomColor === false) {
        ...
        let selection = await mba.selectImage("Boots of Winding Path", colors, `<b>Choose Color:</b>`, "value");
    ...
    await mba.playerDialogMessage(game.user);
    let position = await mba.aimCrosshair(workflow.token, 15, workflow.item.img, interval, workflow.token.document.width);
    await mba.clearPlayerDialogMessage();
