...
async function chromaticBolt({ speaker, actor, token, character, item, args, scope, workflow }) {
    ...
    let selection = await mba.selectImage('Chromatic Bolt', choices, `<b>Choose damage type:</b>`, "both");
    if (!selection) {
...
export let koboldScaleSorcerer = {
    'chromaticBolt': chromaticBolt
}