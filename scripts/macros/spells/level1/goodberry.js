export async function goodberry({speaker, actor, token, character, item, args, scope, workflow}) {
    ...
    const newItemData = {
        ...
        misc: { goodberry: { expirationTime: expirationTime } },
        itemacro: {
            macro: {
            data: {
                _id: null,
    ...
    if (game.modules.get("about-time")?.active) {
        ...
        await newItem.setFlag("misc", "goodberry", goodberryEvent);
    }
    }
    ...
    const i18nGoodberry = initLocalization();
    const expirationTime = getProperty(gbWorkflow.item, "flags.misc.goodberry.expirationTime") ?? 0;
    ...
    if (game.modules.get("about-time")?.active) {
        if (gbWorkflow.item.getFlag("misc", "goodberry")?.actorUuid !== gbWorkflow.actor.uuid) {
        const eventId = game.Gametime.doAt(
            expirationTime,
            deleteGoodberries,
        ...
        await gbWorkflow.item.setFlag("misc", "goodberry", goodberryEvent);
        }
    }
    ...
    const itemsToDelete = actor.itemTypes.consumable.filter(
        (it) =>
        getProperty(it, "flags.misc.goodberry.expirationTime") &&
        now >= getProperty(it, "flags.misc.goodberry.expirationTime")
    );
