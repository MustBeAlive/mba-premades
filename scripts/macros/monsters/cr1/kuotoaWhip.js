...
async function grapple({ speaker, actor, token, character, item, args, scope, workflow }) {
    ...
    async function effectMacroDelSource() {
        let targetDoc = await fromUuid(effect.flags['mba-premades']?.feature?.kuotoaWhip?.grapple?.targetUuid);
        let targetEffect = await mbaPremades.helpers.findEffect(targetDoc.actor, "Kuo-toa Whip: Grapple");
