...
async function item({ speaker, actor, token, character, item, args, scope, workflow }) {
    ...
    featureData.name = "Dominate Person: Assume Control";
    featureData.img = workflow.item.img;
    setProperty(featureData, "flags.midi-qol.onUseMacroName", "[postActiveEffects]function.mbaPremades.macros.dominatePerson.control");
    let castLevel = workflow.castData.castLevel;
    let duration;
    switch (castLevel) {
        case 5: duration = 60; break;
        case 6: duration = 600; break;
        case 7: duration = 3600; break
        case 8:
        case 9: duration = 28800; break;
    }
    async function effectMacroDelSource() {
        await warpgate.revert(token.document, "Dominate Person");
    };
    async function effectMacroDelTarget() {
        Sequencer.EffectManager.endEffects({ name: `${token.document.name} DomPer` });
        let casterDoc = await fromUuid(effect.flags['mba-premades']?.spell?.dominatePerson?.casterUuid);
        if (!casterDoc) return;
        ...
        let controlEffect = await mbaPremades.helpers.findEffect(token.actor, "Dominate Person: Lost Control");
    ...
    let effectDataSource = {
        ...
        'description': `
            ...
            <p>While the person is charmed, you have a telepathic link with it, as long as the two of you are on the same plane of existence.</p>
    ...
    let effectDataTarget = {
        ...
        'changes': [
            ...
            },
            {
                ...
                'mode': 0,
                'value': 'function.mbaPremades.macros.dominatePerson.damaged,isDamaged',
                'priority': 20
            }
        ],
        'flags': {
    ...
    new Sequence()
        ...
        .persist()
        .name(`${target.document.name} DomPer`)
...
async function damaged({ speaker, actor, token, character, item, args, scope, workflow }) {
    let effect = await mba.findEffect(token.actor, "Dominate Person: Target");
    if (!effect) {
        ui.notifications.warn("Unable to find effect! (Dominate Person: Target)");
        return;
    }
    let featureData = await mba.getItemFromCompendium("mba-premades.MBA Spell Features", "Dominate: Save", false);
    if (!featureData) return;
    delete featureData._id;
    featureData.name = "Dominate Person: Save";
    featureData.img = effect.icon;
    featureData.system.save.dc = effect.flags['mba-premades']?.spell?.dominatePerson?.saveDC;
...
async function control({ speaker, actor, token, character, item, args, scope, workflow }) {
    let effect = await mba.findEffect(workflow.actor, "Dominate Person");
    if (!effect) {
        ui.notifications.warn("Unable to find the effect! (Dominate Person)");
        return;
    }
    let targetDoc = await fromUuid(effect.flags['mba-premades']?.spell?.dominatePerson?.targetUuid);
    let target = targetDoc.object;
    async function effectMacroDel() {
        Sequencer.EffectManager.endEffects({ name: `${token.document.name} DomPAC` })
    };
    let effectData = {
        'name': "Dominate Person: Lost Control",
