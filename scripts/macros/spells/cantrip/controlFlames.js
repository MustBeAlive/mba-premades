...
export async function controlFlames({ speaker, actor, token, character, item, args, scope, workflow }) {
    ...
    const captureArea = {
        ...
        y: position.y,
        scene: canvas.scene,
        radius: 5 / canvas.scene.grid.distance * canvas.grid.size
    };
    const [lightDoc] = warpgate.crosshairs.collect(captureArea, 'AmbientLight')
    if (!lightDoc) {
        ui.notifications.info("Unable to find document, try again!");
        return;
    }
    let lightHidden = lightDoc.hidden;
    if (lightHidden === true) return;
    ...
    var ntc = {
        name: function (color) {
            ...
            return (cl < 0 ? ["#000000", "Invalid Color: " + color, false] : [lightColor]);
        },
        hsl: function (color) {
    ...
    do {
        let choicesType = [
            [`Extinguish Light (${lightHidden})`, "off"],
            [`Light Brightness (Current: ${lightBright} bright/${lightDim} dim)`, "brightness"],
            [`Light Color (${ntc.name(lightColor)})`, "color"],
            ["Apply Changes", "apply"],
        ...
        if (selectionType === "off") {
            ...
            else if (lightHidden === false) lightHidden = true;
        }
        if (selectionType === "brightness") {
            let choicesBrightness = [
            ...
            if (selectionBrightness === "half") {
                lightBright = Math.min(lightBright / 2);
                lightDim = Math.min(lightDim / 2);
            }
        }
        ...
        if (selectionType === "apply") i++;
        if (selectionType === "cancel") return;
    } while (i < 1);
    ...
    let size = Math.min(lightDim / 4);
    new Sequence()

        .effect()
        .file("jb2a.markers.light.intro.yellow02")
        .atLocation(position)
