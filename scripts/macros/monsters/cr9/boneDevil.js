...
async function poison({ speaker, actor, token, character, item, args, scope, workflow }) {
    ...
    if (mba.findEffect(target.actor, "Bone Devil: Poison")) return;
    let saveDC = workflow.item.system.save.dc;
    async function effectMacroDel() {
        Sequencer.EffectManager.endEffects({ name: `${token.document.name} BoDePoi` })
    };
    const effectData = {
    ...
    new Sequence()
        ...
        .mask(target)
        .persist()
        .name(`${target.document.name} BoDePoi`)
