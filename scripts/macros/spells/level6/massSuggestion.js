...
async function cast({ speaker, actor, token, character, item, args, scope, workflow }) {
    ...
    for (let target of targets) {
        if (mba.findEffect(target.actor, "Deafened") || mba.checkTrait(target.actor, "ci", "charmed")) {
            ChatMessage.create({
                flavor: `<u>${target.document.name}</u> is unaffected by Mass Suggestion!`,
...
async function item({ speaker, actor, token, character, item, args, scope, workflow }) {
    ...
    switch (workflow.castData.castLevel) {
        case 6: duration = 86400; break;
        case 7: duration = 86400 * 10;
        case 8: duration = 86400 * 30; break;
        case 9: duration = 86400 * 366; break;
    }
    async function effectMacroDel() {
        Sequencer.EffectManager.endEffects({ name: `${token.document.name} MasSug` })
    };
    let effectData = {
        ...
        'description': `
            <p>You are affected by Mass Suggestion.</p>
    ...
    for (let target of Array.from(workflow.failedSaves)) {
        new Sequence()
            ...
            .mask(target)
            .persist()
            .name(`${target.document.name} MasSug`)
