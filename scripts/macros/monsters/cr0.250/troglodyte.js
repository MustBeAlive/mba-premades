...
async function stenchAuraCombatStart(token, origin) {
    let duplicates = await mba.findNearby(token, 500, "any", true, false).filter(t => t.name === "Troglodyte");
    ...
    async function effectMacroDel() {
        ...
        await mbaPremades.macros.monsters.troglodyte.stenchAuraEnd(token);
    };
    let effectData = {
        'name': "Troglodyte: Stench Aura",
        'icon': "modules/mba-premades/icons/generic/stench.webp",
        'origin': origin.uuid,
        'changes': [
            {
                'key': 'flags.mba-premades.aura.troglodyteStenchAura.name',
                'mode': 5,
                'value': "troglodyteStenchAura",
                'priority': 20
            },
            {
                'key': 'flags.mba-premades.aura.troglodyteStenchAura.range',
                'mode': 5,
                'value': "5",
                'priority': 20
            },
            {
                'key': 'flags.mba-premades.aura.troglodyteStenchAura.disposition',
                'mode': 5,
                'value': "all",
                'priority': 20
            },
            {
                'key': 'flags.mba-premades.aura.troglodyteStenchAura.effectName',
                'mode': 5,
                'value': "Troglodyte: Stench",
                'priority': 20
            },
            {
                'key': 'flags.mba-premades.aura.troglodyteStenchAura.macroName',
                'mode': 5,
                'value': "troglodyteStenchAura",
                'priority': 20
            },
            {
                'key': 'flags.mba-premades.aura.troglodyteStenchAura.conscious',
                'mode': 5,
                'value': "true",
                'priority': 20
            },
            {
                'key': 'flags.mba-premades.aura.troglodyteStenchAura.spellDC',
                'mode': 5,
                'value': 12,
                'priority': 20
            },
    ...
    let flagAuras = {
        'troglodyteStenchAura': {
            'name': 'troglodyteStenchAura',
            'range': 5,
            'disposition': 'all',
            'effectName': 'Troglodyte: Stench',
            'macroName': 'troglodyteStenchAura',
            'conscious': true,
            'spellDC': 12
        }
    };
...
async function stenchAura(token, selectedAura) {
    ...
    let effectData = {
        ...
        'description': `
            <p>At the start of your next turn, you will be subjected to Troglodyte's Stench.</p>
        `,
        'flags': {
            'dae': {
                'showIcon': true,
...
async function stenchAuraTurnStart(token) {
    let effect = await mba.findEffect(token.actor, "Troglodyte: Stench");
    let troglodyteDoc = await fromUuid(effect.flags['mba-premades']?.originUuid);
    let featureData = await mba.getItemFromCompendium("mba-premades.MBA Monster Features", "Troglodyte Stench: Save", false);
    if (!featureData) return;
    let feature = new CONFIG.Item.documentClass(featureData, { 'parent': troglodyteDoc.actor });
    ...
    if (!featureWorkflow.failedSaves.size) {
        let effectDataImmune = {
            ...
            'description': `
                <p>You are immune to Troglodyte's Stench for the next hour.</p>
            `,
            'duration': {
                'seconds': 3600
            }
        ...
        async function effectMacroDel() {
            Sequencer.EffectManager.endEffects({ name: `${token.document.name} TroPoi` })
        };
        let effectData = {
            'name': "Troglodyte: Poison",
            'icon': "modules/mba-premades/icons/generic/generic_poison.webp",
            'description': `
                <p>You are poisoned by Troglodyte's Stench until the start of your next turn.</p>
            `,
            'changes': [
                {
                    'key': 'macro.CE',
        ...
        new Sequence()
            ...
            .mask()
            .persist()
            .name(`${token.document.name} TroPoi`)
...
async function stenchAuraEnd(token) {
    effectAuras.remove('troglodyteStenchAura', token.document.uuid);
}
