...
export async function loadTriggers() {
    triggers = game.settings.get('mba-premades', 'Movement Triggers');
    if (game.user.isGM) {
        for (let name of Object.values(triggers)) {
            for (let spell of name) {
                let effect = await fromUuid(spell.effectUuid)
                if (!effect) {
                    console.log('MBA Premades | Removing stale movement trigger for ' + spell.macro);
...
export async function updateGMTriggers(updatedTriggers) {
    await game.settings.set('mba-premades', 'Movement Triggers', updatedTriggers);
}
...
export async function tokenMovedEarly(token, updates, options, userId) {
    ...
    setProperty(options, 'mba-premades.coords.previous.x', token.x);
    setProperty(options, 'mba-premades.coords.previous.y', token.y);
    setProperty(options, 'mba-premades.coords.previous.elevation', token.elevation);
}
...
export async function tokenMoved(token, changes, options, userId) {
    ...
    for (let name of Object.values(triggers)) {
        ...
        for (let spell of name) {
            ...
            let distance = mba.getDistance(token.object, sourceToken, spell.wallsBlock);
            if (distance > spell.range || distance === -1) continue;
            if (spell.offTurnMoveSpecial && mba.inCombat()) {
                if (game.combat.current.tokenId != token.id) {
                    let oldDistance = mba.getCoordDistance(sourceToken, {
                        'width': token.width,
                        'height': token.height,
                        'x': options['mba-premades'].coords.previous.x,
                        'y': options['mba-premades'].coords.previous.y,
                        'elevation': options['mba-premades'].coords.previous.elevation
                    });
    ...
    if (token) {
        for (let name of Object.values(triggers)) {
            ...
            for (let spell of name) {
                ...
                let distance = mba.getDistance(token, sourceToken, spell.wallsBlock);
                if (distance > spell.range || distance === -1) continue;
                validSources.push(spell);
            }
            let maxLevel = Math.max(...validSources.map(spell => spell.castLevel));
    ...
    if (lastToken) {
        for (let name of Object.values(triggers)) {
            ...
            for (let spell of name) {
                ...
                let distance = mba.getDistance(lastToken, sourceToken);
