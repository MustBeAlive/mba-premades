import Heap from "../../heap/priorityQueue.js";

let allQueues = {};

function add(uuid, key, priority) {
    if (!game.settings.get('mba-premades', 'Priority Queue')) return;
...
async function wait(uuid, key) {
    if (!game.settings.get('mba-premades', 'Priority Queue')) return true;
    let currentKey = allQueues[uuid].peek()[1];
    let stacks = 0;
    while (currentKey != key) {
        stacks++;
        if (stacks >= 300) {
            console.error('MBA | Queue time took too long and was removed!');
