...
async function petrifyingGazeTrigger(actor, token) {
    ...
    let [target] = await mba.findNearby(token, 30, null, false, false).filter(i => i.document.id === tokenId && i.document.name != "Basilisk");
    if (!target) return;
    if (mba.checkTrait(target.actor, "ci", "petrified")) return;
    if (mba.findEffect(target.actor, "Petrified")) return; // just overly cautious
    if (mba.findEffect(target.actor, "Basilisk: Petrifying Gaze")) return;
    if (mba.findEffect(target.actor, "Basilisk: Petrification")) return;
    let choices = [["<u>Avert Eyes</u> (inability to attack Basilisk)", "avert"], ["<u>Embrace Petrifying Gaze</u> (saving throw)", "save"]];
    let selection = await mba.remoteDialog("Basilisk: Petrifying Gaze", choices, mba.firstOwner(target).id, `<p>You are about to be affected by</p><p><u>Basilisk's Petrifying Gaze</u></p><p>What would you like to do?</p>`);
    ...
    if (selection === "avert") {
        const effectDataAvert = {
            ...
            'description': `
                ...
                <p>If you look at the basilisk in the meantime, you must immediately make the save.</p>
            `,
            'changes': [
                {
                    'key': 'flags.midi-qol.onUseMacroName',
                    'mode': 0,
                    'value': 'function.mbaPremades.macros.monsters.basilisk.avertEyes,preItemRoll',
                    'priority': 20
                }
            ],
            'flags': {
        ...
        if (!mba.findEffect(target.actor, "Basilisk: Avert Eyes")) await mba.createEffect(target.actor, effectDataAvert);
        return;
    }
    if (mba.findEffect(target.actor, "Basilisk: Avert Eyes")) return;
...
async function petrifyingGazeItem({ speaker, actor, token, character, item, args, scope, workflow }) {
    ...
    async function effectMacroDel() {
        ...
        async function effectMacroDel() {
            await Sequencer.EffectManager.endEffects({ name: `${token.document.name} Basilisk Petrification`, object: token })
        }
        const effectData = {
        ...
        new Sequence()
            ...
            .zIndex(1)
            .persist()
            .name(`${token.document.name} Basilisk Petrification`)
