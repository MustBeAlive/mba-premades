	...
	switch (selection) {
		...
		case 'Wedding': {
			...
			async function effectMacroDel() {
				let effect = await chrisPremades.helpers.findEffect(actor, 'Ceremony: AC Bonus');
				if (effect) await chrisPremades.helpers.removeEffect(effect);
			}
			async function effectMacroEachTurn() {
				let effect = await chrisPremades.helpers.findEffect(actor, 'Ceremony: Wedding');
				let bonusEffect = await chrisPremades.helpers.findEffect(actor, 'Ceremony: AC Bonus');
				...
				if (nearbyPartner.length && bonusEffect) return;
				if (nearbyPartner.length && !bonusEffect) {
					await chrisPremades.helpers.createEffect(actor, effectData);
					return;
				}
				if (!nearbyPartner.lenth && bonusEffect) {
					await chrisPremades.helpers.removeEffect(bonusEffect);
					return
				} else return;
			}
			const effectData1 = {
				...
				'flags': {
					...
					'effectmacro': {
						...
						'onEachTurn': {
							'script': chrisPremades.helpers.functionToString(effectMacroEachTurn)
						}
					},
					'midi-qol': {
						'castData': {
