...
async function web({ speaker, actor, token, character, item, args, scope, workflow }) {
    ...
    async function effectMacroDel() {
        Sequencer.EffectManager.endEffects({ name: `${token.document.name} Ettercap Web` })
    };
    const effectData = {
        'name': "Ettercap: Web",
        ...
        'origin': workflow.item.uuid,
        'description': `
            <p>You are restrained by Ettercap's webbing.</p>
    ...
    new Sequence()
        ...
        .mask()
        .persist()
        .name(`${target.document.name} Ettercap Web`)
