...
async function cast({ speaker, actor, token, character, item, args, scope, workflow }) {
    for (let target of Array.from(workflow.targets)) {
		if (mba.checkTrait(target.actor, "ci", "charmed")) {
			ChatMessage.create({
				flavor: `<u>${target.document.name}</u> is unaffected by Charm Monster!`,
...
async function item({ speaker, actor, token, character, item, args, scope, workflow }) {
	...
	await warpgate.wait(100);
	let featureData = await mba.getItemFromCompendium("mba-premades.MBA Spell Features", "Charm Monster: Save", false);
	...
	async function effectMacroDel() {
		Sequencer.EffectManager.endEffects({ name: `${token.document.name} ChaMon` })
	}
	let effectData = {
	...
	for (let target of Array.from(featureWorkflow.failedSaves)) {
		new Sequence()
			...
			.playbackRate(1)
			.persist()
			.name(`${target.document.name} ChaMon`)
