...
async function biteCheck({ speaker, actor, token, character, item, args, scope, workflow }) {
    let effect = workflow.actor.effects.find(e => e.name.includes(`${workflow.token.document.name}: Grapple`) && e.name != "Grappled");
    if (!effect) return;
    let target = workflow.targets.first();
    if (effect.name.includes(target.document.name)) return;
    ui.notifications.warn(`You can only attack currently grappled target! (check passive effects)`);
    return false;
}
