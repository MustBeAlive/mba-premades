...
// To do: upgrade macro when 3.1+ (armor will get "mgc" property)

async function corrodeArmor({ speaker, actor, token, character, item, args, scope, workflow }) {
    if (!workflow.hitTargets.size) return;
    let target = workflow.targets.first();
    let armorNames = ["Breastplate", "Chain Mail", "Chain Shirt", "Half Plate", "Plate", "Plate Armor", "Ring Mail", "Scale Mail", "Spiked", "Splint"];
    let validArmor = target.actor.items.filter(i => i.system.equipped === true && armorNames.some(armorName => i.name.includes(armorName)) && !i.name.includes("+1") && !i.name.includes("+2") && !i.name.includes("+3"));
    if (!validArmor.length) return;
    let targetArmor;
    if (validArmor.length === 1) targetArmor = validArmor[0];
    if (!targetArmor) [targetArmor] = await mba.selectDocument("Which armor is the right one?", validArmor);
    ...
    if (!corrosionEffect) {
        ...
        if (targetArmor.system.armor.value - 1 <= 10) {
            ChatMessage.create({
                content: `<p><u>${targetArmor.name}</u> is destroyed by Gray Ooze's Armor Corrosion!</p>`,
                speaker: { actor: workflow.actor }
            });
            targetArmor.delete();
            return;
