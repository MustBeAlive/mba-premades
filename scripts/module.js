...
Hooks.once('ready', async function() {
    ...
    if (game.settings.get('mba-premades', 'Chwinga')) Hooks.on('midi-qol.preTargetDamageApplication', macros.monsters.chwinga.nineLives);
    ...
    if (game.settings.get('mba-premades', 'Globe of Invulnerability')) Hooks.on('midi-qol.preambleComplete', macros.globeOfInvulnerability.target);
    if (game.settings.get('mba-premades', 'Elusive')) Hooks.on('midi-qol.preAttackRoll', macros.elusive.hook);
    if (game.settings.get('mba-premades', 'Goblin Boss')) Hooks.on('midi-qol.preAttackRoll', macros.monsters.goblinBoss.redirectAttack);
    if (game.settings.get('mba-premades', 'Interception')) Hooks.on('midi-qol.preTargetDamageApplication', macros.fightingStyle.interception);
    if (game.settings.get('mba-premades', 'Kuo-toa')) Hooks.on('midi-qol.AttackRollComplete', macros.monsters.kuotoa.stickyShield);
    if (game.settings.get('mba-premades', 'Lich')) Hooks.on('midi-qol.preTargetDamageApplication', macros.monsters.lich.lairTetherOnHit);
    if (game.settings.get('mba-premades', 'Merregon')) Hooks.on('midi-qol.AttackRollComplete', macros.monsters.merregon.loyalBodyguard);
    if (game.settings.get('mba-premades', 'Mimic')) Hooks.on('midi-qol.AttackRollComplete', macros.monsters.mimic.adhesivePassive);
    ...
    if (game.settings.get('mba-premades', 'Protection')) Hooks.on('midi-qol.preAttackRoll', macros.fightingStyle.protection);
    ...
    if (game.settings.get('mba-premades', 'Quickling')) Hooks.on('midi-qol.preAttackRoll', macros.monsters.quickling.blurredMovement);
    ...
    Hooks.on("dnd5e.preRollHitDie", macros.contamination.contaminationHitDie);
    Hooks.on("dnd5e.preRollHitDie", diseases.diseaseHitDie);
    Hooks.on("dnd5e.preRollHitDie", macros.items.potionOfVitality.hitDie);
    Hooks.on("dnd5e.preLongRest", macros.contamination.contaminationPreLongRest);
    Hooks.on("dnd5e.restCompleted", macros.contamination.contaminationAfterLongRest);
